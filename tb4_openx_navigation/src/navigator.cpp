...
void Navigator::WaitUntilNav2Active()
{
  ...
  // CHANGE "dfq/" TO ROBOT NAMESPACE (default = "")
...
bool Navigator::Spin(double spin_dist)
{
  ...
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::Spin>::SharedPtr>, this, _1);
...
bool Navigator::GoToPose(const geometry_msgs::msg::Pose::SharedPtr pose)
{
  ...
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::NavigateToPose>::SharedPtr>, this, _1);
...
bool Navigator::FollowPath(const nav_msgs::msg::Path::SharedPtr path)
{
  ...
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::FollowPath>::SharedPtr>, this, _1);
...
bool Navigator::FollowWaypoints(const std::vector<geometry_msgs::msg::PoseStamped>& poses) {
  ...
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::FollowWaypoints>::SharedPtr>, this, _1);
...
bool Navigator::Backup(double backup_dist,double backup_speed)
{
  ...
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::BackUp>::SharedPtr>, this, _1);
...
bool Navigator::get_path_internal(const geometry_msgs::msg::Pose::SharedPtr pose){
  ...
  // Update according to new API: Change `pose` to `goal`
  goal_msg.goal.pose.position = pose->position;
  goal_msg.goal.pose.orientation = pose->orientation;
  
  // Optional: Specify `use_start = false` so it takes the current robot pose
  goal_msg.use_start = false;

  auto send_goal_options = rclcpp_action::Client<nav2_msgs::action::ComputePathToPose>::SendGoalOptions();
  
  send_goal_options.goal_response_callback =
    std::bind(&Navigator::generic_goal_response_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::ComputePathToPose>::SharedPtr>, this, _1);
...
void Navigator::wait_for_node_to_activate(const std::string& node_name)
{
  ...
  while (!client->wait_for_service(3s)) {
    RCLCPP_WARN(this->get_logger(), "Waiting for %s service to be available...", node_name.c_str());
  }
  ...
  while (state != "active") {
    ...
    auto future_status = rclcpp::spin_until_future_complete(this->get_node_base_interface(), future, 5s);

    if (future_status == rclcpp::FutureReturnCode::SUCCESS) {
      auto response = future.get();
      if (response) {
        state = response->current_state.label;
        RCLCPP_INFO(this->get_logger(), "%s is now in state: %s", node_name.c_str(), state.c_str());
      }
    } else {
      RCLCPP_ERROR(this->get_logger(), "Failed to get state for %s", node_name.c_str());
      return; 
    }
  }
}
